...
!$acc data present(density, energy,u,p,r,Mi,w,z,Kx,Ky)
...
!$acc parallel loop collapse(2) independent present(p,w,Kx,Ky)
    DO k=y_min,y_max
        DO j=x_min,x_max
            w(j, k) = (1.0_8                                      &
                + ry*(Ky(j, k+1) + Ky(j, k))                      &
...
!$acc parallel loop collapse(2) independent present(u,p,r,Mi,w,z)
    DO k=y_min,y_max
        DO j=x_min,x_max
...
!$OMP PARALLEL
  IF (preconditioner_on) THEN
!$OMP DO
!$acc parallel loop collapse(2) independent present(z,p,r)
    DO k=y_min,y_max
        DO j=x_min,x_max
