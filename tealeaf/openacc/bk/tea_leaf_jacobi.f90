...
!$acc data present(celldx,celldy,volume,density,energy,u0,u1,un,Kx_tmp,Ky_tmp,Kx,Ky)
!$acc kernels
...
!$acc data present(un,u1,u0,kx,ky)
...
!$acc parallel loop collapse(2) independent present(u,density, energy)
!$OMP PARALLEL DO 
  DO k=y_min, y_max
    DO j=x_min, x_max
...
!$acc parallel loop independent collapse(2) private(smvp)
    DO k=y_min, y_max
      DO j=x_min, x_max
        smvp = (1.0_8                                         &
            + ry*(Ky(j, k+1) + Ky(j, k))                      &
